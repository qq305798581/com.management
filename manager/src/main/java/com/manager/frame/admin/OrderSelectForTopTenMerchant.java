...
public class OrderSelectForTopTenMerchant extends JFrame {
	 ...
	 * Create the frame.
	 */
	public OrderSelectForTopTenMerchant() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Integer.valueOf(endmonth.getSelectedItem().toString())) {
					...
					if(Integer.valueOf(startmonth.getSelectedItem().toString())>=
						...
						List<Merchant> res = mapper.selectAllProductAmountBymerchantBetweenSomeTime(start, end, 10);
						for(int i=0;i<res.size();i++) {
							model.addRow(new String[] {
									String.valueOf(res.get(i).getMId()),
									res.get(i).getMAccount(),
									res.get(i).getMName(),
									res.get(i).getMPhone()
							});
						}
