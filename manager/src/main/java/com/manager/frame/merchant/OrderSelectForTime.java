...
public class OrderSelectForTime extends JFrame {
	 ...
	 * Create the frame.
	 */
	public OrderSelectForTime(Merchant mer) {
		this.merchant = mer;
			...
			public void actionPerformed(ActionEvent e) {
				...
				Integer.valueOf(endmonth.getSelectedItem().toString())) {
						...
						if(rdbtnNewRadioButton.isSelected()) {
							long res = mapper.selectAllOrderQuantityBetweenSomeTime(start, end, merchant);
							JOptionPane.showMessageDialog(contentPane, "该段时间内订单数量为"+res, "",
									JOptionPane.INFORMATION_MESSAGE);
						}else {
							long res = mapper.selectAllProductAmountBetweenSomeTime(start, end, merchant);
