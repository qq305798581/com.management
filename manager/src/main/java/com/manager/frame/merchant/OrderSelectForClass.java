...
public class OrderSelectForClass extends JFrame {
	 ...
	 * Create the frame.
	 */
	public OrderSelectForClass(Merchant mer) {
		this.merchant = mer;
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for(int i=0,count=0;i<sl.size();i++) {
					if(sl.get(i).getFId()==comboBox.getSelectedIndex()) {
						if(count==comboBox_1.getSelectedIndex()) {
							si=i;
							break;
						}else {
							count++;
				...
				if(rdbtnNewRadioButton.isSelected()) {
					long res = mapper.selectProductQuantityBySecondclassBetweenSomeTime(si, merchant);
					JOptionPane.showMessageDialog(contentPane, "该类别下订单数量为"+res, "",
							JOptionPane.INFORMATION_MESSAGE);
				}else {
					long res = mapper.selectProductAmountBySecondclassBetweenSomeTime(si, merchant);
